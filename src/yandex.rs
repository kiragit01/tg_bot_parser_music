...
fn classify_input(text: &str) -> Result<PlaylistInput> {
    ...
    bail!(
        "Не распознана ссылка. Поддерживается:\n\
         • Ссылка на плейлист Яндекс.Музыки\n\
         • Iframe-код (кнопка «Код для вставки» в ЯМ)\n\
         • Или отправь текстовый список: Исполнитель - Название"
    )
}
    ...
    match input {
        ...
        PlaylistInput::Uuid { url } => {
            ...
            match resolve_uuid_playlist(&url).await {
                ...
                Ok(_) | Err(_) => {
                    bail!(
                        "Не удалось загрузить этот плейлист.\n\n\
                         Попробуй:\n\
                         1. «Поделиться» → «Код для вставки» → отправь iframe-код\n\
                         2. Скопируй треки текстом (Исполнитель - Название) и отправь списком"
                    )
                }
            }
        }
